...
public class Aoc2
{
   ...
   public static void main(final String[] args) throws IOException
   {
      ...
      try (var resource = Aoc2.class.getResourceAsStream("/aoc2-input.txt");
            var input = new BufferedReader(new InputStreamReader(resource)))
      {
         long total = 0;
         final var g = app.readGames(input);

         final List<Integer> possibles = app.sumPossible(g);
         for (final Integer value : possibles)
         {
            total += value.intValue();
         }
         System.out.println("Ao2 total = " + total);
      }
   }
    ...
    */
   Aoc2(final int red, final int green, final int blue)
   {
